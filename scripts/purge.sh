...
echo -e "-=|[ Lara-Stacker |> Container |> PURGE ]|=-"
...
if ! ensureDockerAccess; then
    prompt "OrbStack's Docker daemon is not reachable." "Open OrbStack and try again." false
fi
...
docker image rm -f caddy:2 redis:7-alpine mysql:8.4 axllent/mailpit:latest minio/minio:latest minio/mc tailscale/tailscale:latest >/dev/null 2>&1 || true
...
for vol in caddy_data caddy_config mysql_data minio_data redis_data tailscale_state; do
