...
if ! dockerCompose exec -T mysql mysql -u root -p"$DB_PASSWORD" -e "CREATE DATABASE IF NOT EXISTS \`$db_name\`;"; then
    prompt "Failed to create database." "Check MySQL container status and retry." false
fi
